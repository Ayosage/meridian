...
describe('handleCreateMatch', () => {
  ...
  it('401 always when the server has no token configured, even on an "empty" match', async () => {
  ...
  it('422 when the room constructor rejects the options (bad counts)', async () => {
    const d = deps({ createRoom: vi.fn(async () => { throw new Error('players must be 3..8') }) })
